...
Mesh* Mesh::generaEstrellaTexCor(GLdouble re, GLuint np, GLdouble h)
{
	...
	GLdouble re_ = 0.5, reI_ = 0.25;

	mesh->vTexCoords.emplace_back(x, y); //abre con vO
	for (size_t i = 1; i <= np; i++)
	{
		//radio exterior
		x = re_ * cos(radians(grados));
		y = re_ * sin(radians(grados));
		mesh->vTexCoords.emplace_back(x, y);
		grados += incremento;

		//radio interior
		xi = reI_ * cos(radians(grados));
		yi = reI_ * sin(radians(grados));
